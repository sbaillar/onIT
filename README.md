...
make build      # dist/onIT (GUI) + dist/teams-busylight (headless CLI)
make firmware   # compile sketch, refresh the image embedded in the app
make pkg        # dist/onIT.pkg installer (bundles esptool for in-app flashing)
make windows    # dist/teams-busylight.exe
```

`teams-busylight -ports` lists serial ports if the device isn't detected
...
- `cmd/onit/` — Fyne menu bar app · `cmd/teams-busylight/` — headless CLI
